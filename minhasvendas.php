...
<form method='post' action='minhasvendas'>
<input type='search' name='bcc' id='buscar_compra' placeholder='Buscar Vendas'>
<label for='buscar_compra'>Buscar Vendas</label>
</form>
</div>"; 

echo "<div class='avisos'>SUAS VENDAS</div>";

$_buscando_compras = $_con->query("SELECT * FROM localcom_vendas as v,localcom_produtos as p WHERE v.id_produto=p.localcom_produto_id and id_vendedor='$_usuario' ORDER BY data_venda DESC");
		...
		if($temPesq){
			...
			for($i=0;$i<$tamanho;$i++){
				...
				if(preg_match("/".$pes[$i]."/i",$_exibir['localcom_produto'])){
					...
					echo "<a href='prod_vendido?venda=".$_exibir['venda_id']."'>
			...
			echo "<a href='produto_vendido?venda=".$_exibir['venda_id']."'><div class='linha_tabela hoverable valign-wrapper'>
